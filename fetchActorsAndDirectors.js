...
async function fetchActorsAndDirectors() {
  ...
  for (const movie of allMovies) {
    try {
      ...
      const actors = response.data.cast.map((actor) => actor.name);
      ...
      console.log(
        `Updated movie: ${movie.title}  ${Math.floor(
          (moviesFetched / allMoviesLength) * 100
        )}% complete`
      );
    } catch (error) {
      console.error(
        "Error fetching actors and directors for movie:",
        movie.title,
        error
      );
    }
  ...
  console.log(`Finished fetching data for ${moviesFetched} movies.`);
  process.exit();
}
